...
class ANTsRegistration(ScriptedLoadableModule):
  ...
  def __init__(self, parent):
    ...
    self.parent.contributors = ["Dženan Zukić (Kitware Inc.), Simon Oxenford (Netstim Berlin)"]
...
class ANTsRegistrationLogic(ITKANTsCommonLogic):
  ...
  def process(self, stages, outputSettings, initialTransformSettings=None, generalSettings=None, wait_for_completion=False):
    ...
    fixedImage = slicer.util.itkImageFromVolume(initialTransformSettings['fixedImageNode'])
    ...
    movingImage = slicer.util.itkImageFromVolume(initialTransformSettings['movingImageNode'])
