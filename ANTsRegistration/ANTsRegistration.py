...
class ANTsRegistrationWidget(ScriptedLoadableModuleWidget, VTKObservationMixin):
    ...
    def populateJacobianInputs(self):
        ...
        for path in filePaths:
            ...
            self.ui.jacobianInputListWidget.addItem(path)
    ...
    def onGenerateJacobianImages(self):
        forwardTransformPaths = [self.ui.jacobianInputListWidget.item(x).text() for x in range(self.ui.jacobianInputListWidget.count)]
        displacementPaths = [x.replace('-forward.tfm', '-displacement.nii.gz') for x in forwardTransformPaths]
        jacobianPaths = [x.replace('-forward.tfm', '-jacobian.nii.gz') for x in forwardTransformPaths]
...
class ANTsRegistrationLogic(ITKANTsCommonLogic):
    ...
    def generateJacobianDeterminantImage(self, path, templatePath, displacementPath, jacobianPath):
        ...
        displacement_field = itk.transform_to_displacement_field_filter(forwardTransform,
                                                                use_reference_image=True,
                                                                reference_image=templateImage)
        
        itk.imwrite(displacement_field, displacementPath, compression=True)
