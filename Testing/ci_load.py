...
Whether Slicer runs with its main window is the workflow's choice (the action's
``main-window`` input).  A module that uses the layout manager in setup() needs
it: under ``--no-main-window`` ``slicer.app.layoutManager()`` is None and the
module raises, looking broken when only the window is missing.  This extension's
modules do not, so its workflows leave the window off.
